...
class DownloadTask implements LoggableTask, Task
{
    ...
    public function run(State $state): void
    {
        ...
        if (!($info instanceof FiasInformerResponse)) {
            throw new TaskException(
                "State parameter '" . StateParameter::FIAS_INFO . "' must be an '" . FiasInformerResponse::class . "' instance for '" . self::class . "'."
            );
        }
        ...
        $this->log(
            LogLevel::INFO,
            "Downloading '{$info->getFullUrl()}' to '{$localFile->getPathname()}'."
        );

        $this->downloader->download($info->getFullUrl(), $localFile);
    }
}
